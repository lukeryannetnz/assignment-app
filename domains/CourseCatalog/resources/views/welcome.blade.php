...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        <div class="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
            <!-- Header -->
            @if (Route::has('identity-access.auth.login'))
                <header class="p-6 lg:p-8">
                    <nav class="flex justify-end gap-4">
                        ...
                        @else
                            <a href="{{ route('identity-access.auth.login') }}" class="px-6 py-2.5 text-white font-medium rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 transition-all duration-300 hover:-translate-y-0.5">
                                Log in
                            </a>
                            @if (Route::has('identity-access.auth.register'))
                                <a href="{{ route('identity-access.auth.register') }}" class="px-6 py-2.5 text-white font-medium rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 transition-all duration-300 hover:-translate-y-0.5">
                                    Register
                                </a>
                            @endif
                        @endauth
            ...
            <main class="flex items-center justify-center px-6 py-12 lg:py-20">
                <div class="max-w-6xl w-full text-center">
                    ...
                    <div class="mb-16 animate-fade-in-down">
                        <x-course-catalog::graduation-cap-logo class="w-32 h-32 mx-auto mb-8 drop-shadow-lg text-white" />
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center items-center animate-fade-in-up-delay">
                        ...
                        @else
                            @if (Route::has('identity-access.auth.login'))
                                <a href="{{ route('identity-access.auth.login') }}" class="px-8 py-4 bg-white text-indigo-600 font-semibold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                    Get Started
                                </a>
                            @endif
                            @if (Route::has('identity-access.auth.register'))
                                <a href="{{ route('identity-access.auth.register') }}" class="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-xl border-2 border-white hover:bg-white/20 hover:-translate-y-1 transition-all duration-300">
                                    Sign Up Free
                                </a>
                            @endif
                        @endauth

            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(24rem,1fr)]">
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    @forelse ($nodes as $node)
                                        <tr class="align-top">
                                            ...
                                            <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ $node['parent_id'] ?? 'Root' }}</td>
                                            <td class="px-6 py-4">
                            ...
                            <form method="POST" action="{{ route('tenancy.admin.org-nodes.update', $node['id']) }}" class="space-y-2 rounded-xl bg-slate-50 p-3 dark:bg-slate-900/40">
                                                            ...
                                                            >
                                                                @foreach ($nodes as $candidateNode)
                                                                    @if ($candidateNode['id'] !== $node['id'])
                                                                        <option value="{{ $candidateNode['id'] }}" @selected($candidateNode['id'] === $node['parent_id'])>
                                                    ...
                                                    >
                                                        ...
                                                        <p class="text-xs text-slate-500">
                                                            {{ $node['is_active'] ? 'Deactivate after child nodes are inactive.' : 'Reactivate once the parent node is active.' }}
                                                        </p>
                                                        <button
                                                            type="submit"
                ...
                <aside class="space-y-6">
                    <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/10">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Add Node</h3>
                        <form method="POST" action="{{ route('tenancy.admin.org-nodes.store') }}" class="mt-4 space-y-4">
                            @csrf
                            <input type="hidden" name="ui_form" value="1">
                            <div>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500"
                                >
                                    <option value="">Root company</option>
                                    @foreach ($nodes as $node)
                                        <option value="{{ $node['id'] }}" @selected((string) $node['id'] === old('parent_id'))>
                    ...
                    <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/10">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    Dry-run the file first, then commit the reviewed result.
                                </p>
                            </div>
                        </div>

                        <form method="POST" action="{{ route('tenancy.admin.org-nodes.imports.dry-run') }}" enctype="multipart/form-data" class="mt-4 space-y-4">
                            @csrf
                            <input type="hidden" name="ui_form" value="1">
                            <div>
                            ...
                            <button type="submit" class="inline-flex items-center rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-900">
